...
namespace OBS_Helper.MacOS.Services.ObsConfig;
...
public sealed class SceneTemplateService
{
    ...
    public SceneTemplateService(ObsConnectionService obs, HostBridge host)
    {
        _obs = obs;
        _host = host;
    }
    ...
    public async Task<IReadOnlyList<SceneTemplate>> LoadAsync()
    {
        ...
        try
        {
            ...
            try
            {
                _cache = LoadEmbedded() ?? new List<SceneTemplate>();
                _loadError = null;
            }
            catch (Exception ex)
            {
    ...
    private static List<SceneTemplate>? LoadEmbedded()
    {
        try
        {
            var asm = Assembly.GetExecutingAssembly();
            var name = asm.GetManifestResourceNames()
                .FirstOrDefault(n => n.EndsWith("scene_templates.json", StringComparison.Ordinal));
            if (name is null) return null;
            using var s = asm.GetManifestResourceStream(name);
            if (s is null) return null;
            using var sr = new StreamReader(s);
            return JsonSerializer.Deserialize<List<SceneTemplate>>(sr.ReadToEnd());
        }
        catch
        {
            return null;
