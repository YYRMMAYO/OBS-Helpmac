...
public sealed class ObsConfigService
{
    ...
    public ObsConfigService(HostBridge host, ObsConnectionService obs, Infrastructure.KeyValueStore store)
    {
        _host = host;
        _obs = obs;
        _store = store;
    ...
    /// <summary>定位 OBS 配置目录；手动指定路径会持久化，重启后仍生效。</summary>
    ...
    private async Task SaveOverrideAsync(string path)
        => await Task.Run(() => _store.Set(OverrideStorageKey, path));

    private async Task<string> GetOverrideAsync()
    {
        var v = _store.Get(OverrideStorageKey);
