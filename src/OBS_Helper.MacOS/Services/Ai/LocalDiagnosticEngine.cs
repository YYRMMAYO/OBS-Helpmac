...
namespace OBS_Helper.MacOS.Services.Ai;

/// <summary>
/// 本地离线诊断引擎（技术计划 §4.5「本地规则引擎」）。
///
/// 不联网、不依赖密钥，输入是「日志分析报告 + 系统体检 + 配置体检 + OBS 实时快照 + 用户问题」，
/// 输出与云端引擎完全一致的 <see cref="DiagnosticResult"/>。逻辑：
/// <list type="number">
///   <li>日志分析发现（最权威、已脱敏）：直接转成诊断项，并关联知识库拿分步方案；</li>
///   <li>系统体检发现：HAGS / 双显卡 / 磁盘余量等日志里看不到的系统级隐患；</li>
///   <li>配置体检发现：直接读 obs-studio 配置目录，属于「还没出事但迟早出事」；</li>
///   <li>用户提问驱动：用 <see cref="AssistantService"/> 在知识库里做关键词匹配；</li>
///   <li>已连接但无日志：从实时性能统计里推导渲染/编码/网络告警；</li>
///   <li>按严重程度排序，生成一段中文结论。</li>
/// </list>
///
/// 四个来源的优先级刻意设计成「日志 &gt; 系统 &gt; 配置 &gt; 提问」：越靠前的证据越硬，
/// 同一个知识库条目只保留最先命中的那条，避免同一件事被重复列三遍。
/// </summary>
public sealed class LocalDiagnosticEngine
{
    ...
    public async Task<DiagnosticResult> DiagnoseAsync(DiagnosticContext ctx, string? query)
    {
        ...
        // 2) 系统体检发现（方向 A）：日志看不见的系统级隐患，如 HAGS / 双显卡 / 磁盘余量
        if (ctx.System is { Available: true })
        {
            foreach (var sf in ctx.System.Findings)
            {
                if (IsDuplicate(items, sf.ProblemId)) continue;

                var item = new DiagnosticItem
                {
                    ProblemId = sf.ProblemId ?? "",
                    Title = sf.Title,
                    Severity = DiagnosticSeverityMapper.Map(sf.Severity),
                    Source = "系统体检",
                    Reason = string.IsNullOrEmpty(sf.Detail) ? sf.Suggestion : sf.Detail,
                    Evidence = sf.Suggestion
                };
                await AttachStepsByIdAsync(item, sf.ProblemId);
                items.Add(item);
            }
        }

        // 3) 配置体检发现（方向 B）：直接读 obs-studio 配置目录，属于「还没出事但迟早出事」
        if (ctx.Config is { Available: true })
        {
            foreach (var cf in ctx.Config.Findings)
            {
                if (IsDuplicate(items, cf.ProblemId)) continue;

                var item = new DiagnosticItem
                {
                    ProblemId = cf.ProblemId ?? "",
                    Title = cf.Title,
                    Severity = DiagnosticSeverityMapper.Map(cf.Severity),
                    Source = "配置体检",
                    Reason = string.IsNullOrEmpty(cf.Detail) ? cf.Suggestion : cf.Detail,
                    Evidence = cf.Suggestion
                };
                await AttachStepsByIdAsync(item, cf.ProblemId);
                items.Add(item);
            }
        }

        // 4) 用户提问驱动的知识库匹配（去重：日志已覆盖的不再重复）
    ...
    /// 同一个知识库条目只呈现一次。
    ///
    /// 举例：日志里出现「Recording Format: mp4」，配置体检也会读到同样的设置，
    /// 两边都关联 rc-mp4corrupt。对用户而言这是同一件事，重复列出只会稀释注意力。
    /// 保留先加入的那条（日志 &gt; 系统 &gt; 配置，越靠前证据越硬）。
    /// </summary>
    private static bool IsDuplicate(List<DiagnosticItem> items, string? problemId)
        => !string.IsNullOrEmpty(problemId) && items.Any(i => i.ProblemId == problemId);

    private async Task AttachStepsByIdAsync(DiagnosticItem item, string? problemId)
    {
        if (string.IsNullOrEmpty(problemId)) return;
        var p = await _problems.GetByIdAsync(problemId);
        if (p is not null) AttachSteps(item, p);
    }
